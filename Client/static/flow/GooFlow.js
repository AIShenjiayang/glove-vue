...
var GooFlow = function(selector, property) {
  ...
  if (navigator.userAgent.indexOf('MSIE 8.0') > 0 || navigator.userAgent.indexOf('MSIE 7.0') > 0 || navigator.userAgent.indexOf('MSIE 6.0') > 0) { GooFlow.prototype.useSVG = '' } else	GooFlow.prototype.useSVG = '1'
  // 初始化区域图的对象
  ...
  if (property.haveGroup) { this._initGroup(width, height) }
  // 为了节点而增加的一些集体绑定
...
GooFlow.prototype = {
    ...
    this.$group.on('mousedown', { inthis: this }, function(e) { // 绑定RESIZE功能以及移动功能
      ...
      document.onmousemove = function(e) {
        ...
        if (cursor !== 'move') {
          ...
          X = ev.x - vX; Y = ev.y - vY
          if (X < t.left - This.$workArea[0].parentNode.scrollLeft) { X = t.left - This.$workArea[0].parentNode.scrollLeft } else if (X + This.$workArea[0].parentNode.scrollLeft + This.$areaData[id].width * This.$scale > t.left + This.$workArea.width()) { X = t.left + This.$workArea.width() - This.$workArea[0].parentNode.scrollLeft - This.$areaData[id].width * This.$scale }
          if (Y < t.top - This.$workArea[0].parentNode.scrollTop) { Y = t.top - This.$workArea[0].parentNode.scrollTop } else if (Y + This.$workArea[0].parentNode.scrollTop + This.$areaData[id].height * This.$scale > t.top + This.$workArea.height()) { Y = t.top + This.$workArea.height() - This.$workArea[0].parentNode.scrollTop - This.$areaData[id].height * This.$scale }
      ...
      document.onmouseup = function() {
        ...
        if (!isMove) return
        if (cursor !== 'move') { This.resizeArea(id, This.$ghost.outerWidth() / This.$scale, This.$ghost.outerHeight() / This.$scale) } else { This.moveArea(id, (X + This.$workArea[0].parentNode.scrollLeft - t.left) / This.$scale, (Y + This.$workArea[0].parentNode.scrollTop - t.top) / This.$scale) }
        return false
      }
    })
    // 绑定修改文字说明功能
  ...
  _initWorkForNode: function() {
    ...
    this.$workArea.on('mousedown', '.ico', { inthis: this }, function(e) {
      ...
      document.onmousemove = function(e) {
        ...
        if (X < t.left - This.$workArea[0].parentNode.scrollLeft) { X = t.left - This.$workArea[0].parentNode.scrollLeft } else if (X + This.$workArea[0].parentNode.scrollLeft + This.$nodeData[id].width * This.$scale > t.left + This.$workArea.width()) { X = t.left + This.$workArea.width() - This.$workArea[0].parentNode.scrollLeft - This.$nodeData[id].width * This.$scale }
        if (Y < t.top - This.$workArea[0].parentNode.scrollTop) { Y = t.top - This.$workArea[0].parentNode.scrollTop } else if (Y + This.$workArea[0].parentNode.scrollTop + This.$nodeData[id].height * This.$scale > t.top + This.$workArea.height()) { Y = t.top + This.$workArea.height() - This.$workArea[0].parentNode.scrollTop - This.$nodeData[id].height * This.$scale }
        This.$ghost.css({ left: X + 'px', top: Y + 'px' })
        isMove = true
      }
      document.onmouseup = function() {
  ...
  _initEditFunc: function(useOperStack) {
    ...
    this.$workArea.mousemove({ inthis: this }, function(e) {
      ...
      if (lineStart) {
        if (GooFlow.prototype.useSVG !== '') {
          ...
          if (line.childNodes[1].getAttribute('marker-end') === 'url("#arrow2")') { line.childNodes[1].setAttribute('marker-end', 'url(#arrow3)') } else	line.childNodes[1].setAttribute('marker-end', 'url(#arrow2)')
    ...
    this.$lineMove.on('mousedown', { inthis: this }, function(e) {
      ...
      document.onmouseup = function() {
        if (isMove) {
          var p = This.$lineMove.position()
          if (This.$lineMove.data('type') === 'lr') { This.setLineM(This.$lineMove.data('tid'), (p.left + 3) / This.$scale) } else if (This.$lineMove.data('type') === 'tb') { This.setLineM(This.$lineMove.data('tid'), (p.top + 3) / This.$scale) }
        }
  ...
  bindHeadBtnEvent: function(funcs) {
    if (this.$head != null) { this.$headBtnEvents = funcs }
  },
  // 每一种类型结点及其按钮的说明文字
  ...
  delNode: function(id, trigger) {
    if (!this.$nodeData[id])	return
    if (trigger !== false && typeof this.onItemDel === 'function' && this.onItemDel(id, 'node') === false)	return
    // 先删除可能的连线
  ...
  addLineDom: function(id, lineData) {
    ...
    // 开始计算线端点坐标
    var res
    if (lineData.type && lineData.type !== 'sl') { res = calcPolyPoints(n1, n2, lineData.type, lineData.M, this.$scale) } else { res = calcStartEnd(n1, n2, this.$scale) }
    if (!res)	return

    if (lineData.type === 'sl') { this.$lineDom[id] = GooFlow.prototype.drawLine(id, res.start, res.end, lineData.marked, lineData.dash, this.$scale) } else { this.$lineDom[id] = GooFlow.prototype.drawPoly(id, res.start, res.m1, res.m2, res.end, lineData.marked, lineData.dash, this.$scale) }
  ...
  addLine: function(id, json) {
    ...
    // 避免两个节点间不能有一条以上同向接连线
    for (var k in this.$lineData) {
      if ((json.from === this.$lineData[k].from && json.to === this.$lineData[k].to && json.dash === this.$lineData[k].dash)) { return }
    }
    // 设置$lineData[id]
    this.$lineData[id] = {}
    if (json.type) {
  ...
  resetLines: function(id, node) {
    for (var i in this.$lineData) {
        ...
        if (this.$lineData[i].type === 'sl') { res = calcStartEnd(node, other, this.$scale) } else { res = calcPolyPoints(node, other, this.$lineData[i].type, this.$lineData[i].M, this.$scale) }
        if (!res)	break
		  } else if (this.$lineData[i].to === id) { // 找开始点
        other = this.$nodeData[this.$lineData[i].from] || null
        if (other == null)	continue
        if (this.$lineData[i].type === 'sl') { res = calcStartEnd(other, node, this.$scale) } else { res = calcPolyPoints(other, node, this.$lineData[i].type, this.$lineData[i].M, this.$scale) }
        if (!res)	break
		  }
		  if (other == null)	continue
  ...
  moveLinePoints: function(lineId, newStart, newEnd, noStack) {
    ...
    if (newStart == null || newStart === '') { newStart = this.$lineData[lineId].from }
    if (newEnd == null || newEnd === '') { newEnd = this.$lineData[lineId].to }

    // 避免两个节点间不能有一条以上同向接连线
    for (var k in this.$lineData) {
      if ((newStart === this.$lineData[k].from && newEnd === this.$lineData[k].to)) { return }
    }
