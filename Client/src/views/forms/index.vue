...
<el-table-column class-name="status-col" :label="'状态'" width="100">
    <template slot-scope="scope">
          <span :class="scope.row.enabled | statusFilter">{{statusOptions.find(u =>u.key == scope.row.enabled).display_name}}</span>
        </template>
</el-table-column>
...
<el-table-column align="center" :label="'操作'" width="230" class-name="small-padding fixed-width">
    <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.enabled!=0" size="mini" type="danger" @click="handleModifyStatus(scope.row,0)">停用</el-button>
        </template>
</el-table-column>
</el-table>
...
<script>
    ...
    export default {
      ...
      methods: {
        ...
        handleModifyStatus(row, enabled) { // 模拟修改状态
          this.$message({
            message: '操作成功',
            type: 'success'
          })
        ...
        handleUpdate(row) { // 弹出编辑框
          this.$router.push('/forms/edit/' + row.id)
        },
