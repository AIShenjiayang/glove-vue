...
$.fn.flowdesign = function(options) {
  ...
  flowPanel.exportDataEx = function() {
    ...
    var fnodes = [] // 会签分流节点
    var hnodes = [] // 会签合流节点
    var startroundFlag = 0 // 开始节点标识
    var endroundFlag = 0 // 结束节点标识
  ...
  flowPanel.SetNodeEx = function(id, data) {
    flowPanel.setName(id, data.name, 'node', data.setInfo)
  }
  ...
  if (options.isprocessing) { // 如果是显示进程状态
    ...
    tipHtml +=
                    '<div style="display: inline-block;"><i style="padding-right:5px;background-color:#5cb85c"></i><span>通过</span></div>'
    tipHtml +=
                    '<div style="display: inline-block;margin-left: 10px;"><i style="padding-right:5px;background-color:#5bc0de;"></i><span>正在处理</span></div>'
    tipHtml +=
                    '<div style="display: inline-block;margin-left: 10px;"><i style="padding-right:5px;background-color:#d9534f;"></i><span>不通过</span></div>'
    tipHtml +=
                    '<div style="display: inline-block;margin-left: 10px;"><i style="padding-right:5px;background-color:#f0ad4e;"></i><span>驳回</span></div>'
    tipHtml +=
                    '<div style="display: inline-block;margin-left: 10px;"><i style="padding-right:5px;background-color:#999;"></i><span>未处理</span></div></div>'
    ...
    $.each(options.nodeData,
      function(i, item) {
        ...
        if (item.type === 'start round mix') {
          ...
          if (item.setInfo !== undefined && item.setInfo !== null && item.setInfo.Taged !== undefined && item.setInfo.Taged !== null) {
        ...
        if (item.setInfo !== undefined && item.setInfo !== null && item.setInfo.Taged !== undefined && item.setInfo.Taged !== null) {
          ...
          $('#' + item.id).hover(function() {
            window.layer.tips(tips, '#' + item.id)
          })
        } else {
          $('#' + item.id).hover(function() {
            window.layer.tips('暂无处理信息', '#' + item.id)
          })
        }
      })
  }
      ...
      function(i, item) {
        if (item.setInfo !== undefined) {
          ...
          if (item.setInfo.NodeDesignateData !== undefined) {
            var _rowstr = ''
            for (i in item.setInfo.NodeDesignateData.roles) {
            ...
            _rowstr = ''
            for (i in item.setInfo.NodeDesignateData.users) {
              _postitem = item.setInfo.NodeDesignateData.users[i]
              // _one = clientuserData[_postitem]
          ...
          var _row = ''
          for (i in item.setInfo.frmPermissionInfo) {
